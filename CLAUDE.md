...
3. **참고 대상은 UX만 모방한다**: Cursor 등 참고 대상에서 따라야 할 것은 **화면·기능(UX)**이지, 그것을 만드는 **내부 구현 수단**이 아니다. 저쪽이 어떤 수단을 쓰는지는 우리가 그 수단을 써도 되는 근거가 되지 않는다. 수단은 아래 4번 기준으로 따로 판단한다.

4. **우리가 직접 제어하는 통신은 우리 자산이다**: 우리가 stdin/stdout으로 claude를 직접 제어하는 것(`control_request`의 interrupt/set_model 등, `backend/src/core/claude-process.ts`)은 "공식 SDK 라이브러리 의존"이 **아니라 우리가 구현한 통신**이다. 그 완성도를 높이는 것은 옳다.

   미문서화 subtype도 **다음 세 가지를 전부 지키면 써도 된다.**

   - **공식 CLI 명령으로 같은 일을 하는 경로가 코드에 살아 있어야 한다.** 문서에 적힌 대체 방법이 아니라, 지금 실행되는 코드여야 한다.
   - **미문서화 경로가 실패하면 자동으로 공식 경로로 내려가야 한다.** 사용자가 무엇이 실패했는지 알 필요가 없어야 한다.
   - **미문서화 경로가 영원히 사라져도 기능이 그대로 동작해야 한다.** 느려지는 것은 괜찮고, 없어지는 것은 안 된다.

   금지되는 것은 미문서화 프로토콜을 **쓰는 것**이 아니라 거기에 **의존하는 것**이다. 위 세 가지를 지키면 의존이 성립하지 않는다.

   적용 사례: MCP 서버 목록은 살아 있는 CLI에 `control_request{subtype:"mcp_status"}`로 먼저 묻고, 물어볼 CLI가 없거나 답이 없으면 공식 `claude mcp list`로 내려간다(`backend/src/core/features/mcp-status.ts`). 이렇게 한 이유는 두 가지다. 첫째, 패널이 보여줘야 하는 것은 **사용자의 채팅이 실제로 붙어 있는 프로세스의 상태**인데, 별도 프로세스를 새로 띄워 찔러본 결과는 그것과 어긋날 수 있다. 둘째, `claude mcp list`와 `claude mcp get`은 매번 CLI를 새로 띄워 모든 stdio 서버를 다시 시작시키고, `docker run`으로 도는 서버는 그때마다 컨테이너를 하나씩 남긴다(#363).

> 판단 기준은 "어느 쪽이 더 안 깨지나"가 아니라 **"이 경로가 사라져도 우리가 그 일을 계속 할 수 있나"**이다. 둘 다 깨진다. 사람이 읽으라고 만든 텍스트 출력도 계약이 아니어서, Windows 콘솔이 `✔`를 `√`로 바꾼 것만으로 상태 판정이 깨진 적이 있고(#319), `claude mcp get`의 출력은 우리 코드가 스스로 신뢰하지 않아 설정 파일에서 다시 읽어 덮어쓴다.
>
> 그러므로 공식 명령을 **1순위 구현 수단**으로 못박지 않는다. 대신 **공식 명령으로 가는 길을 항상 열어두고 실제로 동작하게 유지한다.** 어느 쪽을 먼저 시도할지는 정확성과 비용으로 정하고, 없어져도 되는 쪽이 어느 쪽인지는 이 원칙이 정한다.
...
### ★ Bridge는 환경↔백엔드의 유일한 양방향 경로 (위반 금지)

Bridge는 실행 환경과 Node 백엔드 사이를 **양쪽 포맷을 노멀라이즈해 중개**하는 장치다. MCP가 그렇듯, 양측은 서로를 직접 알지 못하고 오직 Bridge를 통해서만 대화한다.

**방향은 Bridge의 정의에 들어가지 않는다. 들어오는 것도 나가는 것도 전부 Bridge를 지난다.**

| 방향 | 규칙 |
|------|------|
| 환경 → 백엔드 | 환경(제트브레인 등)이 백엔드에 무언가를 전달하는 **유일한 경로는 Bridge**다 |
| 백엔드 → 환경 | 백엔드가 환경을 제어하는 **유일한 방법은 Bridge**다 |

#### 모든 Bridge는 동일한 기능을 구현한다

`Bridge` 인터페이스에 선언된 기능은 **모든 구현체가 빠짐없이 달성**해야 한다. `BrowserBridge`가 "IDE가 없으니 우린 못 한다"로 빠지는 것은 원칙 위반이다. 그 환경에서 그 기능을 **어떤 수단으로 달성할지**를 찾는 것이 Bridge 구현의 일이다.

- 제트브레인: IDE가 이미 아는 것(VFS 이벤트 등)을 **받아서 번역·전달**한다
- 터미널(standalone): 중개할 호스트가 없으므로 **그 환경에서 가능한 수단으로 직접 달성**한다

두 번째를 "Bridge가 스스로 일한다"고 오해하지 말 것. 같은 계약을 각 환경의 방식으로 이행하는 것이며, 이것이 백엔드가 **어느 환경에 붙어 있는지 몰라도 되게** 만드는 장치다.

> **이 원칙이 만들어진 배경**: #359에서 파일 변경 감지를 제트브레인 경로에만 직접 배선하고 Bridge 인터페이스에 올리지 않았다. 그 결과 **standalone 사용자에게는 기능이 통째로 없었고**, 브라우저에서 검수하기 전까지 아무도 몰랐다. 인터페이스에 올렸다면 `BrowserBridge`가 구현하지 않은 채로 넘어갈 수 없었다 — 인터페이스는 빠진 자리를 드러내주는 장치인데 그것을 쓰지 않은 것이다.

**진짜 no-op만 no-op으로 둔다.** "에디터 탭 열기"처럼 그 개념 자체가 IDE 안에만 존재하는 것은 브라우저에서 no-op이 맞다. 반면 "파일이 바뀐 걸 안다"처럼 **어느 환경에서나 의미가 성립하는 기능은 no-op으로 둘 수 없다.** 판별 질문: *"이 기능이 없으면 그 환경 사용자가 손해를 보는가?"* 그렇다면 no-op이 아니라 미구현이다.
...
같은 이유로 **`ccg` 명령어 이름과 `CCG_*` 환경변수 29종도 개명하지 않는다.** `ccg`는 사용자가 직접 타이핑하고 PATH에 등록해 쓰는 이름이라, 바꾸면 이미 설치된 사용자의 명령이 사라진다. `CCG_*`도 공개된 계약이므로 리브랜딩을 이유로 건드리지 않는다.
