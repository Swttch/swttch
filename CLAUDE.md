...
## ★ 진입점부터 만들고 안쪽으로 들어간다 (구현 순서, 위반 금지)

**기능을 만들 때는 사용자가 직접 트리거할 진입점부터 만든다.** 안쪽 구현이 어떻게 될지 몰라도 상관없다.

`@@` 멘션 기능을 예로 들면 순서는 이렇다.

1. `@@`를 입력했을 때 **패널이 뜨는 것**을 가장 먼저 만든다. 이 단계에서는 빈 패널이라도 뭔가를 띄우는 것이 중요하다.
2. "데이터가 들어온다면 이렇게 채워야지" 싶은 **가짜 더미데이터를 인라인으로 박아** 목록 UI를 먼저 만든다.
3. 방향키 이동과 선택, 선택 시 인풋에 칩으로 붙는 것까지 **여기서** 만든다.
4. 칩이 백스페이스로 한 번에 지워지도록 만든다.
5. 그 다음에 더미데이터를 지우고 **실제 백엔드에서 데이터를 불러오도록** 바꾼다.

### 왜 이 순서인가

**잔조정 요구는 생각보다 많은 경우에 UI에 관한 것이다.** 화면이 눈앞에 서 있어야 그 조정이 싸다.

안쪽부터 만들면 조정을 받을 시점에 이미 밑단이 굳어 있어서, UI 한 줄 바꾸자는 말이 백엔드까지 번진다.

### 판별 규칙

- **"프론트부터 백으로"는 계층 순서가 아니라 위 순서를 뜻한다.** 타입 → 백엔드 → 유틸 → 컴포넌트 → 훅 순으로 만들면 화면에는 끝까지 아무것도 안 뜬다. 실제로 그렇게 해서 지적받은 적이 있다.
- **더미데이터는 인라인으로 박는다.** 별도 fixture 파일이나 mock 계층을 만들지 않는다. 5단계에서 지울 것이기 때문이다.
- **백엔드와 타입 정의는 맨 마지막이다.** 더미의 모양이 곧 계약이므로, 그때는 필요한 필드가 이미 확정돼 있다.
- **매 단계가 끝날 때 화면에서 눈으로 확인 가능해야 한다.** 확인 불가능한 단계를 만들었다면 순서가 틀린 것이다.

이 원칙은 유저 스토리 단위로 수직 작업하라는 규칙과 짝을 이룬다. 스토리가 **무엇을** 자를지를 정하고, 이 원칙이 그 안에서 **어떤 순서로** 만들지를 정한다.
