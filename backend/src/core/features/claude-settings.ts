...
// ─── Proxy Env ─────────────────────────────────────────────────────────────
//
// There used to be an allow-list of eight proxy variable names here, and a function that
// copied just those out of the settings `env` block. It is gone: a list of names is a list
// that has to grow, and it grew twice under exactly the same bug report. A proxy set only in
// settings.json never reached `ccb` (#181), the list gained four names; the lower-case
// spelling of the same variable still did not reach it (#432), the list gained four more.
//
// features/settings-env.ts reads the block without looking at the names, and both children
// now read the file themselves — `claude` always did, and `ccb` does as of
// @swttch/extend-kit 0.7.1. Nothing is left for a ninth name to fall through.
