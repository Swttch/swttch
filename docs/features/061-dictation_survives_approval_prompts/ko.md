...
![승인 창 위 알림줄에 "Recording stopped after a silence."라는 문구와 "Resume recording" 버튼, 그리고 알림을 닫는 X 버튼이 있다](./assets/stopped-by-silence.png)

**이어서 녹음**을 누르면 그 자리에서 다시 시작합니다. 마이크 버튼을 찾으러 승인을 먼저 처리할 필요가 없습니다. 오른쪽 **X**를 누르면 알림만 닫힙니다.

**앞서 말한 내용은 지워지지 않습니다.** 새로 말한 것이 초안 끝에 이어 붙으므로, 하던 말을 그대로 계속하시면 됩니다. 마이크와 연결은 새로 열리지만 글은 그대로 남습니다.
