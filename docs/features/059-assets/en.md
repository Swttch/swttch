# Every image you attached, in one place
...
They stay **among themselves** and never run on into images you already sent.

They are not part of the conversation yet, so counting them together with sent ones would blur what you are about to send.

### 3. The Assets screen

The **image icon** in the header opens every image attached in this session.
...
> **No icon?** Click `⋮` at the end of the header and switch on **Assets** with the eye button. A newly shipped item starts hidden.
...
| See every image in the session | Yes | Yes |
| How far the arrow keys reach | **The images attached to that message** | **The whole session** |

Every image is visible on the Assets screen either way. What is held back is not seeing them, but **keeping one enlarged and paging through the entire session from there.**

When you reach the last image of a message, you are told how many more the session holds. Sponsorship lives in Settings › Sponsor.
...
If you rewound or forked the conversation, only the images on the **branch still in play** appear. Images on an abandoned branch are not in the conversation either.
