...
## Claude가 일하는 중에 보낸 메시지

섹션의 세 번째 행은 다른 순간을 다룹니다. 이미 무언가를 보냈고, Claude가 그것을 하고 있는데, 다시 입력하는 순간입니다.

| 선택지 | 보낸 메시지가 하는 일 |
| --- | --- |
| **대기열에 추가** | 지금 턴이 끝날 때까지 기다렸다가 갑니다. 기본값이고, 지금까지 늘 이렇게 동작했습니다. |
| **현재 작업 조정** | 지금 턴을 끝내고 이 메시지가 대신 답변됩니다. |

조정은 별도의 통로가 아닙니다. Stop 버튼이 보내는 것과 같은 인터럽트이고, 터미널 사용자가 메시지를 치고 Esc를 누르는 것과 같습니다. 보낸 메시지는 이미 CLI의 큐에 들어가 있으므로, 턴을 끝내면 CLI가 그 메시지를 집어 새 턴을 시작합니다. 그때까지 한 일은 대화에 그대로 남아 있어서, Claude는 방금 알아낸 것을 전부 알고 답합니다.

### 메시지 하나만 반대로 보내기

설정을 바꾸지 않고도 메시지 하나만 반대로 보낼 수 있습니다. 그 키는 보내기 단축키를 따라갑니다.

| 보내기 단축키 | 일회성 키 |
| --- | --- |
| Enter | ⌘+Enter (Windows와 Linux에서는 Ctrl+Enter) |
| ⌘+Enter | ⇧⌘+Enter |
| Ctrl/Cmd가 없는 사용자 설정 조합 | 그 조합에 Ctrl/Cmd를 더한 것 |
| Ctrl/Cmd는 있고 Shift는 없는 사용자 설정 조합 | 그 조합에 Shift를 더한 것 |

보내기 단축키가 이미 둘 다 들고 있으면 더할 것이 없으므로 일회성 키도 없습니다. 그때는 행이 일회성 키를 아예 언급하지 않습니다. 동작하지 않을 키의 이름을 대는 것보다 낫기 때문입니다.

개행 단축키가 위 표의 조합과 같은 자리에 있을 때도 마찬가지입니다. 직접 고르신 키가 그 자리를 지킵니다. 고른 적 없는 키가 채워 넣은 설정을 빼앗지는 못합니다.
...
**조정은 턴을 끝내는 것이지 잠시 멈추는 것이 아닙니다.** Claude가 계속 일하는 동안 쪽지를 건네는 방법은 없습니다. CLI에 그런 통로가 없고, 없는 것을 만들어 쓰려면 미문서화된 무언가에 기대야 하는데 그것은 예고 없이 멈출 수 있습니다. 조정이 하는 일은 사람이 손으로 할 수 있는 것을 더 빠르게 하는 것입니다. 턴을 끝내고, 그때까지 알아낸 것을 전부 대화에 남긴 채 다시 묻습니다.
