...
![프롬프트 수정 화면. 이름 필드에 "Reproduce a report"가 있고, 카테고리 줄에는 칩 세 개("Review"와 "Debug"는 파랗게 채워져 있고 "Docs"는 아니다)가 있으며, 내용 상자에는 "Reproduce issue #{{number}} exactly as the reporter wrote it, on {{platform}}. Do not paraphrase their steps."가 적혀 있다. 아래에 Cancel과 Save가 있다.](./assets/edit-prompt.png)
