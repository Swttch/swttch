      ...
      >
        ...
        >
          ...
          <div className="relative flex-shrink-0 px-6 pt-10 pb-6 border-b border-border-default bg-gradient-to-b from-surface-overlay via-surface-overlay to-surface-raised">
            ...
            <p className="relative mt-1.5 text-center text-sm text-text-tertiary">
              <span className="font-mono text-text-secondary">v{release.version}</span>
              <span className="mx-2 opacity-50">·</span>
              {formatReleaseDateShort(release.cdate, t('whatsNew.unknownDate'))}
            </p>

            {/* The promo sits inside the hero, centred, rather than as a
                full-bleed tinted strip under it: that strip is the single most
                recognisable part of the dialog we are deliberately not copying.
                Its copy is still to be decided; the key is in place so it can
                be written without touching this layout. */}
            <div className="relative mt-5 flex flex-wrap items-center justify-center gap-x-3 gap-y-2">
              <span className="text-xs text-text-tertiary">{t('whatsNew.promo')}</span>
              <button
                onClick={() => void getAdapter().openUrl(REPO_URL)}
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-border-default text-xs font-medium text-text-secondary hover:text-text-primary hover:border-accent-primary hover:bg-surface-hover transition-colors"
              >
          ...
          {/* Body: the marketplace note verbatim, sanitized */}
          ...
          {/* Footer: paging on the start edge, the dismiss on the end edge.
              Navigation and the way out are different kinds of action, so they
              sit apart rather than sharing a centred row. */}
          <div className="flex items-center gap-2 px-6 py-4 border-t border-border-default flex-shrink-0">
            <button
              onClick={() => setIndex((i) => Math.max(0, i - 1))}
              disabled={isFirst}
              aria-label={t('whatsNew.newer')}
              className="w-8 h-8 flex items-center justify-center rounded-md text-text-secondary hover:bg-surface-hover transition-colors disabled:opacity-25 disabled:hover:bg-transparent"
            >
              <ChevronLeftIcon className="w-4 h-4 rtl:-scale-x-100" />
            </button>
            <span data-testid="whats-new-pager" className="text-xs tabular-nums text-text-tertiary">
            ...
            <button
              onClick={onClose}
              className="ms-auto px-4 py-1.5 rounded-md text-sm font-medium bg-accent-primary hover:bg-accent-primary-hover text-accent-primary-fg transition-colors"
            >
              {t('whatsNew.close')}
            </button>
          </div>
