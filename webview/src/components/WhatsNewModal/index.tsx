  ...
  return (
    <Portal>
      ...
      >
        ...
        >
          {/* Header: title, the version this page is showing, its date, close */}
          <div className="flex items-center gap-3 px-6 pt-5 pb-4 flex-shrink-0">
            <h2 className="text-xl font-semibold text-text-primary">{t('whatsNew.title')}</h2>
            ...
            <button
              ...
              className="ms-auto w-8 h-8 flex items-center justify-center rounded text-text-tertiary hover:text-text-primary hover:bg-surface-hover transition-colors"
            >
              <XMarkIcon className="w-5 h-5" />
            </button>
          </div>

          {/* Full-width promo strip. Its copy is still to be decided; the key is
              in place so it can be written without touching this layout. */}
          <div className="flex items-center gap-3 px-6 py-2.5 border-y border-banner-info-border bg-banner-info-bg flex-shrink-0">
          ...
          {/* Footer: one release per page, newest first */}
          <div className="flex items-center px-6 py-4 border-t border-border-default flex-shrink-0">
            <button
              onClick={() => setIndex((i) => Math.max(0, i - 1))}
              disabled={isFirst}
              aria-label={t('whatsNew.newer')}
              className="w-10 h-10 flex items-center justify-center rounded-lg border border-border-default text-text-secondary hover:bg-surface-hover transition-colors disabled:opacity-30 disabled:hover:bg-transparent"
            >
              <ChevronLeftIcon className="w-5 h-5 rtl:-scale-x-100" />
            </button>
            <span data-testid="whats-new-pager" className="flex-1 text-center text-sm text-text-tertiary">
