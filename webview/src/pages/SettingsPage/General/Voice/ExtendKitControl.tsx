...
export function ExtendKitControl() {
  ...
  // Shared with the onboarding checklist, which installs the same kit through
  // the same mutation — see {@link runKitInstall} for why the failure path in
  // particular is not duplicated.
  const run = (successKey: 'installed' | 'updated') => runKitInstall(install, successKey);
